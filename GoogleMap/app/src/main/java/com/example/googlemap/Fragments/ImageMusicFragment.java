...
import com.example.googlemap.Classes.PostVK;
import com.example.googlemap.Classes.Post;
import com.example.googlemap.Adapters.PostAdapter;
...
public class ImageMusicFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (fromVK) {
            ...
            if (!MainActivity.databaseHelper.readUser("Пост из вк").equals("-1"))
                isEmpty = false;
            PostVK asyncTask = new PostVK(isEmpty);
            asyncTask.execute();
    ...
    public void Edit(String imgString, String mscString, String author,  String title,
                     String description, String date, String location, double[] latLng, int position){
        Post post = new Post(imgString, mscString, author, title, description, date, location, latLng);
        MainActivity.databaseHelper.update(position, imgString, imgString, title, description, location,
                String.valueOf(latLng[0]), String.valueOf(latLng[1]));
